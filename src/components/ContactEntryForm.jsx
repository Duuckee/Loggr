// Inline contact entry supports both fast logging and step-by-step guidance.
...
export default function ContactEntryForm({
  ...
  // Build the same contact shape used by storage, sync, editing, and export.
  ...
  function handleGuidedNext(event) {
    ...
    setErrors({})
    setGuidedStep((current) => Math.min(2, current + 1))
  }

  const callsignField = (
    <div className="field callsign-field">
      <label htmlFor="contact-callsign">Callsign</label>
      <div className="input-action"><input ref={callsignRef} id="contact-callsign" value={callsign} onChange={(event) => { setCallsign(event.target.value.toUpperCase()); setDuplicate(null); setSavedMessage('') }} placeholder="VK3ABC" autoComplete="off" autoFocus={experienceMode === 'guided' && guidedStep === 0} /><button type="button" className="btn-ghost" onClick={handleLookup} disabled={lookupState.status === 'loading' || !callsign}>Lookup</button></div>
  ...
  const locationFields = (
    <div className="entry-details-grid guided-location-grid">
      <div className="signal-detail-fields"><div className="field"><label>Active operator</label><input value={operator} disabled /></div><label className="checkbox-row"><input type="checkbox" checked={isP2p} onChange={(event) => setIsP2p(event.target.checked)} />Park-to-park — draw a map link</label></div>
      <div>{contactLocationFields}</div>
    </div>
  )

  return (
    <section className={`contact-composer ${experienceMode === 'guided' ? 'guided-composer' : ''}`} aria-labelledby="contact-entry-title">
      <header className="composer-header">
        <div><span className="panel-kicker">{experienceMode === 'guided' ? 'Guided entry' : 'Quick entry'}</span><h2 id="contact-entry-title">Log a contact</h2></div>
      ...
      {experienceMode === 'guided' ? (
        <form className="guided-entry-form" onSubmit={guidedStep < 2 ? handleGuidedNext : handleSubmit}>
          <div className="composer-progress" aria-label={`Step ${guidedStep + 1} of 3`}><span className={guidedStep >= 0 ? 'active' : ''}>1 · Callsign</span><span className={guidedStep >= 1 ? 'active' : ''}>2 · Signal</span><span className={guidedStep >= 2 ? 'active' : ''}>3 · Details</span></div>

          {guidedStep === 0 && <div className="guided-step"><div className="guided-copy"><span className="step-badge">Step 1 of 3</span><h3>Who did you contact?</h3><p>Enter the callsign exactly as you heard it. Lookup can fill their location when available.</p></div>{callsignField}</div>}

          {guidedStep === 1 && <div className="guided-step"><div className="guided-copy"><span className="step-badge">Step 2 of 3</span><h3>Record the signal</h3><p>Choose the band and mode, then check the sent and received reports.</p></div><div className="guided-signal-grid"><div className="field"><label htmlFor="contact-band">Band</label><select id="contact-band" value={band} onChange={(event) => changeBand(event.target.value)}>{BANDS.map((item) => <option key={item}>{item}</option>)}</select></div>{frequencyField}<div className="field"><label htmlFor="contact-mode">Mode</label><select id="contact-mode" value={mode} onChange={(event) => changeMode(event.target.value)}>{MODES.map((item) => <option key={item}>{item}</option>)}</select></div><div className="field"><label htmlFor="rst-sent">RST sent</label><input id="rst-sent" inputMode="numeric" value={rstSent} onChange={(event) => setRstSent(event.target.value)} />{errors.rstSent && <div className="field-error">{errors.rstSent}</div>}</div><div className="field"><label htmlFor="rst-received">RST received</label><input id="rst-received" inputMode="numeric" value={rstReceived} onChange={(event) => setRstReceived(event.target.value)} />{errors.rstReceived && <div className="field-error">{errors.rstReceived}</div>}</div></div></div>}

          {guidedStep === 2 && <div className="guided-step"><div className="guided-copy"><span className="step-badge">Step 3 of 3</span><h3>Add location details</h3><p>This step is optional unless the contact is park-to-park. Save when the contact is ready.</p></div>{locationFields}</div>}
          ...
          <div className="guided-entry-actions">{guidedStep > 0 && <button type="button" className="btn-ghost" onClick={() => { setErrors({}); setGuidedStep((current) => current - 1) }}>Back</button>}<button className="btn-primary" type="submit">{guidedStep < 2 ? 'Continue' : 'Add QSO'} <span aria-hidden="true">→</span></button></div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="quick-entry-grid">
            {callsignField}
            <div className="field"><label htmlFor="contact-band">Band</label><select id="contact-band" value={band} onChange={(event) => changeBand(event.target.value)}>{BANDS.map((item) => <option key={item}>{item}</option>)}</select></div>
            {frequencyField}
            <div className="field"><label htmlFor="contact-mode">Mode</label><select id="contact-mode" value={mode} onChange={(event) => changeMode(event.target.value)}>{MODES.map((item) => <option key={item}>{item}</option>)}</select></div>
            <button className="btn-primary composer-submit" type="submit">Add QSO <span aria-hidden="true">↵</span></button>
          </div>
          {lookupState.message && <div className={`lookup-message composer-message ${lookupState.status}`}>{lookupState.message}</div>}
          {savedMessage && <div className="composer-saved" role="status"><span className="mini-dot live" /> {savedMessage}</div>}
          <details className="entry-details"><summary><span>Additional details</span><small>RST · park · location · notes</small></summary><div className="entry-details-grid"><div className="signal-detail-fields"><div className="field-row"><div className="field"><label htmlFor="rst-sent">RST sent</label><input id="rst-sent" inputMode="numeric" value={rstSent} onChange={(event) => setRstSent(event.target.value)} />{errors.rstSent && <div className="field-error">{errors.rstSent}</div>}</div><div className="field"><label htmlFor="rst-received">RST received</label><input id="rst-received" inputMode="numeric" value={rstReceived} onChange={(event) => setRstReceived(event.target.value)} />{errors.rstReceived && <div className="field-error">{errors.rstReceived}</div>}</div></div><div className="field"><label>Active operator</label><input value={operator} disabled /></div><label className="checkbox-row"><input type="checkbox" checked={isP2p} onChange={(event) => setIsP2p(event.target.checked)} />Park-to-park — draw a map link</label></div><div>{contactLocationFields}</div></div></details>
          {duplicate && <div className="warning-box composer-duplicate" role="alert"><strong>Possible duplicate</strong><span>{duplicate.callsign} was logged on {duplicate.band} {duplicate.mode} at {new Date(duplicate.timestamp).toISOString().slice(11, 16)}Z.</span><div><button type="button" className="btn-ghost" onClick={() => setDuplicate(null)}>Cancel</button><button type="button" className="btn-danger" onClick={() => handleSubmit(null, true)}>Save anyway</button></div></div>}
        </form>
      )}
    </section>

...
| FR06 | Autosave and database upload | `storage.js` saves every active update; `sync.js` optionally syncs using anonymous auth and owner-scoped RLS | Manual reload/offline check; Supabase requires configuration |
...
| C03 | Secure transfer/storage | HTTPS services; secrets server-side; anonymous private account and owner-scoped Supabase RLS | Configuration review before deployment |
...
- Cloud sync is disabled safely until Supabase URL/key, tables, authentication, and owner-approved RLS policies are configured.
